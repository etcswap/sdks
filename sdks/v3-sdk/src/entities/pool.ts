import { BigintIsh, CurrencyAmount, Price, Token } from '@etcswapv2/sdk-core'
...
import { NO_TICK_DATA_PROVIDER_DEFAULT, TickDataProvider } from './tickDataProvider'
...
export class Pool {
   ...
   * @param tickDataProvider The current state of the pool ticks
   */
  public constructor(
    tokenA: Token,
    tokenB: Token,
  ...
  public async getOutputAmount(
    ...
    return [
      CurrencyAmount.fromRawAmount(outputToken, JSBI.multiply(outputAmount, JSBI.BigInt(-1))),
  ...
  public async getInputAmount(
    ...
    const { amountCalculated: inputAmount, sqrtRatioX96, liquidity, tickCurrent } = await this.swap(
      zeroForOne,
      JSBI.multiply(outputAmount.quotient, JSBI.BigInt(-1)),
      sqrtPriceLimitX96
    )
  ...
  private async swap(
    ...
    if (zeroForOne) {
      invariant(JSBI.greaterThan(sqrtPriceLimitX96, MIN_SQRT_RATIO), 'RATIO_MIN')
      invariant(JSBI.lessThan(sqrtPriceLimitX96, this.sqrtRatioX96), 'RATIO_CURRENT')
    } else {
      invariant(JSBI.lessThan(sqrtPriceLimitX96, MAX_SQRT_RATIO), 'RATIO_MAX')
    ...
    while (JSBI.notEqual(state.amountSpecifiedRemaining, ZERO) && state.sqrtPriceX96 !== sqrtPriceLimitX96) {
      ...
      const [sqrtPriceX96, amountIn, amountOut, feeAmount] = this.computeSwapStep(
        state.sqrtPriceX96,
        (zeroForOne
      ...
      if (JSBI.equal(state.sqrtPriceX96, step.sqrtPriceNextX96)) {
        ...
        if (step.initialized) {
          let liquidityNet = (await this.tickDataProvider.getTick(step.tickNext)).liquidityNet
          // if we're moving leftward, we interpret liquidityNet as the opposite sign
          if (zeroForOne) liquidityNet = JSBI.multiply(liquidityNet, JSBI.BigInt(-1))
  ...
  private computeSwapStep(
    ...
    const zeroForOne = JSBI.lessThanOrEqual(sqrtRatioCurrentX96, sqrtRatioTargetX96) === false
    ...
    if (exactIn) {
      const amountRemainingLessFee = JSBI.divide(
        JSBI.multiply(amountRemaining, JSBI.BigInt(1_000_000 - feePips)),
      ...
      if (JSBI.greaterThanOrEqual(JSBI.multiply(amountRemaining, JSBI.BigInt(-1)), amountOut)) {
        sqrtRatioNextX96 = sqrtRatioTargetX96
      } else {
        sqrtRatioNextX96 = SqrtPriceMath.getNextSqrtPriceFromOutput(
          sqrtRatioCurrentX96,
          liquidity,
          JSBI.multiply(amountRemaining, JSBI.BigInt(-1)),
          zeroForOne
        )
      }
    }
    ...
    if (!exactIn && JSBI.greaterThan(amountOut!, JSBI.multiply(amountRemaining, JSBI.BigInt(-1)))) {
      amountOut = JSBI.multiply(amountRemaining, JSBI.BigInt(-1))
    }
    ...
    if (exactIn && JSBI.notEqual(sqrtRatioNextX96, sqrtRatioTargetX96)) {
      ...
      feeAmount = JSBI.divide(
        JSBI.multiply(amountIn!, JSBI.BigInt(feePips)),
        JSBI.subtract(JSBI.BigInt(1_000_000), JSBI.BigInt(feePips))
      )
    }
