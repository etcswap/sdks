...
function multiplyIn256(x: JSBI, y: JSBI): JSBI {
  ...
  return JSBI.bitwiseAnd(product, JSBI.BigInt('0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff'))
}

function addIn256(x: JSBI, y: JSBI): JSBI {
  const sum = JSBI.add(x, y)
  return JSBI.bitwiseAnd(sum, JSBI.BigInt('0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff'))
}

/**
 * Provides functions for computing prices from sqrtRatioX96 values
 */
export abstract class SqrtPriceMath {
  ...
  /**
   * Returns amount0 for a given liquidity and sqrt price range
   */
  ...
  /**
   * Returns amount1 for a given liquidity and sqrt price range
   */
  ...
  /**
   * Returns the next sqrt price given an input amount
   */
  public static getNextSqrtPriceFromInput(
    ...
    invariant(JSBI.greaterThan(sqrtPX96, ZERO), 'SQRT_PRICE')
    invariant(JSBI.greaterThan(liquidity, ZERO), 'LIQUIDITY')
  ...
  /**
   * Returns the next sqrt price given an output amount
   */
  ...
  private static getNextSqrtPriceFromAmount0RoundingUp(
    ...
    if (add) {
      ...
      const product = multiplyIn256(amount, sqrtPX96)
      invariant(JSBI.equal(JSBI.divide(product, amount), sqrtPX96), 'PRODUCT_OVERFLOW')
      invariant(JSBI.greaterThan(numerator1, product), 'NUMERATOR')
  ...
  private static getNextSqrtPriceFromAmount1RoundingDown(
    ...
    if (add) {
      ...
      invariant(JSBI.greaterThan(sqrtPX96, quotient), 'SQRT_PRICE')
