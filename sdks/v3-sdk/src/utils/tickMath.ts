...
import { MAX_SQRT_RATIO, MAX_TICK, MIN_SQRT_RATIO, MIN_TICK, ONE, ZERO } from '../constants'
...
/**
 * Provides functions for computing square root prices from ticks and vice versa
 */
export abstract class TickMath {
  ...
  public static getSqrtRatioAtTick(tick: number): JSBI {
    ...
    if (tick > 0) ratio = JSBI.divide(JSBI.BigInt(-1), ratio)
  ...
  public static getTickAtSqrtRatio(sqrtRatioX96: JSBI): number {
    invariant(
      JSBI.greaterThanOrEqual(sqrtRatioX96, MIN_SQRT_RATIO) && JSBI.lessThan(sqrtRatioX96, MAX_SQRT_RATIO),
      'SQRT_RATIO'
    )
...
function mostSignificantBit(x: JSBI): number {
  ...
  invariant(JSBI.lessThanOrEqual(x, JSBI.BigInt(-1)), 'MAX')
